...
#   Evaluate the model with the best parameters and get the associated predictions
    model = Lasso(best_params["alpha"]).fit(X_train, y_train)
...
#   Load parameters to tune the model
    params = load_parameters("parameters\\lasso_tuning_params.json")
    print("\nSelected parameters to tune the model :\n",params,"\n")
#   Train, optimize and save the model, and display results on test data
    best_params, test_results, rmse, mape = train_test_model(df_final,789,params,scaler,num_col,"parameters\\lasso_best_params","models\\lasso_best_model")
...
#   Export results on test data
    dir_result = "results\\lasso_results_"+date.today().strftime("%Y%m%d")+".csv"
